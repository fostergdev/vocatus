...
class GradesController extends GetxController {
  ...
  Future<void> loadAllGrades() async {
    try {
      ...
      Get.dialog(CustomErrorDialog(title: 'Erro', message: 'Erro ao carregar horários: ${e.toString()}'));
    } finally {
  ...
  Future<void> loadClassesAndDisciplinesForFilters() async {
    try {
      ...
      Get.dialog(CustomErrorDialog(title: 'Erro', message: 'Erro ao carregar dados de filtro: ${e.toString()}'));
    }
  }
  ...
  Future<void> createGrade(Grade grade) async {
    try {
      ...
      if (selectedClasseForForm.value?.id == null) {
        Get.dialog(CustomErrorDialog(title: 'Erro', message: 'Selecione uma turma para o horário.'));
        return;
      }
      ...
      Get.dialog(CustomErrorDialog(title: 'Erro ao Adicionar Horário', message: e.toString()));
    } finally {
      isLoading.value = false;
    }
  }
  ...
  Future<void> updateGrade(Grade grade) async {
    try {
      isLoading.value = true;
      if (grade.id == null) {
        Get.dialog(CustomErrorDialog(title: 'Erro', message: 'ID do horário é nulo. Não foi possível atualizar.'));
        return;
      }
      if (selectedClasseForForm.value?.id == null) {
        Get.dialog(CustomErrorDialog(title: 'Erro', message: 'Selecione uma turma para o horário.'));
      ...
      Get.dialog(CustomErrorDialog(title: 'Erro ao Atualizar Horário', message: e.toString()));
    } finally {
      isLoading.value = false;
    }
  }
  ...
  Future<void> toggleGradeStatus(Grade grade) async {
    try {
      isLoading.value = true;
      if (grade.id == null) {
        Get.dialog(CustomErrorDialog(title: 'Erro', message: 'ID do horário é nulo. Não foi possível mudar o status.'));
        return;
      }
      await _gradeRepository.toggleGradeActiveStatus(grade);
      await loadAllGrades();
    } catch (e) {
      Get.dialog(CustomErrorDialog(title: 'Erro ao Mudar Status do Horário', message: e.toString()));
    } finally {
      isLoading.value = false;
    }
  }
  ...
  void fillEditGradeFields(Grade grade) {
    selectedClasseForForm.value = availableClasses.firstWhereOrNull((c) => c.id == grade.classeId);
    selectedDisciplineForForm.value = availableDisciplines.firstWhereOrNull((d) => d.id == grade.disciplineId);
